...
@Component({
  selector: 'app-a-generatequote',
  templateUrl: './a-generatequote.component.html',
  styleUrls: ['./a-generatequote.component.scss']
})
export class AGeneratequoteComponent {
  po:any=[];
  idd:any=[];
  getuserdata:any=[];
...
this.getpo();
    ...
    getpo(){
      this.api4.get_accounts_po().subscribe({
        next:(data) =>{
          // if(data.length === 0 ){
    ...
    get_po(){
     ...
     view_po(id:any){
      ...
      this.router.navigate(['/pages/accountsviewpo'], navigationExtras);
     }

     search(){
      this.api4.get_po_by_po_id(this.form.value.po_id).subscribe({
        next:(data) =>{
          console.log(data);
          this.result = data;
          this.show = true;
